...
def export(file,
                        ...
                        if is_uv:
                            ...
                            for i in polygons_group:
                                for lidx in mesh_polygons[i].loop_indices:
                                    fw('%.*f %.*f ' % (texcoord_precision,mesh_loops_uv[lidx].uv[0],texcoord_precision,mesh_loops_uv[lidx].uv[1]))
                            fw('" />\n')
    ...
    def export_object(ident, obj_main_parent, obj_main, obj_children):
        ...
        (free,derived) = create_derived_objects(depsgraph, obj_main)  # Blender 2.93 requires obj_main not being in a list and create_derived_object returns this (free,derived) tuple. Later blender versions probably will need to uncomment the prior twon lines instead.
