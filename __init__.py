...
bl_info = {
    "name": "Web3D X3D/VRML2 format (highres texture export)",
    "author": "Campbell Barton, Bart, Bastien Montagne, Seva Alekseyev; highres modifications by Stephen D. Holland",
    "version": (2, 3, 0),
    "blender": (2, 93, 0),
...
def menu_func_import(self, context):
    self.layout.operator(ImportX3D.bl_idname,
                         text="X3D hires Extensible 3D (.x3d/.wrl)")


def menu_func_export(self, context):
    self.layout.operator(ExportX3D.bl_idname,
                         text="X3D hires Extensible 3D (.x3d)")
